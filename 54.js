...
var orderString = 'AKQJT98765432';

var RoyalFlush = /AKQJT/;
var Straights = [];

for(var i = 0; i < 9; i++) {
    Straights.push(new RegExp(orderString.substring(i, i+5)))
}

var FourOfKind = /(.)\1\1\1/;
var FullHouses = [
    /(.)\1\1(.)\2/,
    /(.)\1(.)\2\2/
];
...
function rank(hand) {
    var flush = Object.values(hand.map(c => /([DCHS])/.exec(c)[1]).reduce((a, c) => {
        a[c] = true;
        return a;
    }, {})).length == 1;

    var ordered = hand.map(c => /([2-9AKQJT])/.exec(c)[1]).sort((a, b) => order[b] - order[a]).join('');

    if (RoyalFlush.exec(ordered) && flush) {
        return { 
            v: 1,
            d: [order[RoyalFlush.exec(ordered)[0][0]]]
        };
    }
    if (Straights.filter(sf => sf.exec(ordered)).length > 0 && flush) {
        return {
            v: 2,
            d: [order[ordered[0]]]
        };
    }
    if (FourOfKind.exec(ordered)) {
        return {
            v: 3,
            d: [order[ordered[2]], ordered[0] == ordered[2] ? order[ordered[4]] : order[ordered[0]]]
        };
    }
    if (FullHouses.filter(fh => fh.exec(ordered)).length > 0) {
        return {
            v: 4,
            d: [order[ordered[2]], ordered[0] == ordered[2] ? order[ordered[4]] : order[ordered[0]]]
        };
    ...
    if (flush) {
        return {
            v: 5,
            d: ordered.split('').map(f => order[f])
        };
    }
    if (Straights.filter(s => s.exec(ordered)).length > 0) {
        return {
            v: 6,
            d: [order[ordered[0]]]
        };
    }
    if (ThreeOfKind.exec(ordered)) {
        return {
            v: 7,
            d: [
                order[ordered[2]], 
                ordered[0] != ordered[2] ? order[ordered[0]] : order[ordered[3]],
                ordered[1] != ordered[2] ? order[ordered[1]] : order[ordered[4]]
            ]
        };
    }
    if (TwoPair.exec(ordered)) {
        var tp = TwoPair.exec(ordered)[1];
        var bp = TwoPair.exec(ordered)[2];
        var rem = ordered.split('').filter(cc => 
            cc != tp && cc != bp
        )[0];
        return {
            v: 8,
    ...
    if (OnePair.exec(ordered)) {
        var tp = OnePair.exec(ordered)[1];
        var rem = ordered.split('').filter(cc => 
            cc != tp
        ).join('');
        return {
...
hands.forEach(h => {
    ...
    if (rank1.v < rank2.v) {
        ...
        if (rank1.v == rank2.v) {
            ...
            var i = 0;
            while (rank1.d[i] == rank2.d[i]) {
                i++;
            }
            if (rank1.d[i] > rank2.d[i]) {
                wins++;
            }
        }
    }
